...
void mpow(sparseMat matrixOne, sparseMat& result, int maxPower);
void toPowerOfZero(const sparseMat,sparseMat& result);
...
int main(int argc, char *argv[])
{
  int power=0;
	...
	if(power==1){
		...
		toPowerOfZero(rows,prod);
		outMat(prod);
	}
	else if(power%2==1)
	{
		power--;
		mpow(rows,prod,power);
		multMat(prod,rows,prod);
		outMat(prod);
	}
	else{	
		mpow(rows,prod,power);
...
void mpow(const sparseMat matrixOne, sparseMat& result, int maxPower){
	if(maxPower ==2)
	{
		multMat(matrixOne,matrixOne, result);
	}
	else	
	{
		maxPower=maxPower/2;
		mpow(matrixOne,result,maxPower);
	}

}
void toPowerOfZero(const sparseMat rows,sparseMat& result){
	for(int i =0;i<rows.size();i++)
	{
		sparseRow row;
		row.push_back(nz(i+1,1));
